...
if ($conn and isset($post->transaccion) and !empty($post->transaccion) and isset($post->trabajo) and !empty($post->trabajo))  {
     // Query
	$qry = odbc_prepare($conn, 'UPDATE dba.ordenserviciodetalles a
		SET a.fechahoraini = now(), 
			a.fechahorafin = dateadd(day, coalesce(c.diasestimados,0),dateadd(hour,coalesce(c.horasestimada,0),dateadd(minute,coalesce(c.minestimada,0),now())))
			FROM ordenserviciodetalles a INNER JOIN ordenserviciostipos c ON a.trabajo = c.idostipos
	WHERE transaccion = ? AND trabajo = ? ');
	  // Get Result
